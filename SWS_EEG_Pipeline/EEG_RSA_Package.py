...
def all_rsa(permutation=False,current_epoch_list = None,return_epoch = False,
                        ...
                        n_jobs = 8,
                        re_sfreq=None,
                        r_type='r2',input_ch=None,if_normalize=True,if_abs=False,reject_thr=None):
...
def rsm_plot(time_window = [-0.2,0.8],
             path = ' ',window_size = 0.1,sample_r = 50,if_fisher_z = False,if_square = False,if_norm = False,layout = [2,5],
    ...
    for i,ax in enumerate(axis.flat):
        ...
        ax.imshow(matrix_plot,origin="lower",cmap=cmap,vmin=vmin,vmax=vmax)
...
def time_similarity_correlation_to_step(scores,conn_ref,if_permutation,if_fisher_z = False,if_square = False,if_norm = False,baseline_fix = None,**kwargs):
    ...
    if baseline_fix is not None:
        if len(baseline_fix)==2:
            ...
            fixer = np.mean(scores[:,:,start_idx:end_idx],axis=2)
            for i in range(scores.shape[2]):
                scores[:,:,i] = scores[:,:,i]-fixer
    if if_norm:
        for i in range(scores.shape[2]):
