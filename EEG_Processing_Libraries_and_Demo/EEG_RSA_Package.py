...
def rsm_plot(time_window = [-0.2,0.8],
             path = ' ',window_size = 0.1,sample_r = 50,if_fisher_z = False,if_square = False,if_norm = False,layout = [2,5],baseline_fix = None,if_norm_plot = False,if_equalize = False,if_equalize_plot = False,
    ...
    if if_equalize:
        for participant_idx in range(scores.shape[0]):
            scores[participant_idx,:,:] = exposure.equalize_hist(scores[participant_idx,:,:], nbins=1024)
    ...
    for i,ax in enumerate(axis.flat):
        ...
        if if_equalize_plot:
            matrix_plot = exposure.equalize_hist(matrix_plot, nbins=1024)
        if if_norm_plot:
...
def build_rsa_trial_df(scores,conn_ref,t=0,col0_repeat=3.0,col1_repeat=3.0,if_trend_up=True,if_trend_sqrt=True):
    ...
    data = []
    distance_vector = np.max(distance_vector) - distance_vector
...
def time_similarity_correlation_to_step(scores,conn_ref,if_permutation,if_fisher_z = False,if_square = False,if_norm = False,baseline_fix = None,**kwargs):
    ...
    ci_low_list = np.zeros([scores.shape[2]])
    ci_up_list = np.zeros([scores.shape[2]])
    if if_fisher_z:
        scores = np.arctanh(scores)
    if if_square:
        scores = scores**2
    ...
    for t in trange(scores.shape[2]):
        ...
        ci_low_list[t] = ci_table.loc['Step', 0]
        ci_up_list[t] = ci_table.loc['Step', 1]
    return t_list,p_list,distance_matrix,r_list,d_list,ci_low_list,ci_up_list
